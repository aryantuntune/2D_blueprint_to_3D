...
def tables(image_path, scale_factor):
    """Detect tables in the floorplan using contour detection"""
    ...
    for contour in contours:
        ...
        # Table: medium size, roughly square or rectangular - STRICT thresholds
        if 5000 < area < 25000 and 0.6 < aspect_ratio < 1.7:
            tables_list.append((x, y, w, h))

    return tables_list


def beds(image_path, scale_factor):
    """Detect beds in the floorplan using contour detection"""
    img = cv2.imread(image_path, 0)
    img = image.cv2_rescale_image(img, scale_factor)
    ...
    for contour in contours:
        ...
        # Bed: larger, more rectangular - STRICT thresholds
        if 15000 < area < 50000 and 1.3 < aspect_ratio < 2.5:
            beds_list.append((x, y, w, h))

    return beds_list


def chairs(image_path, scale_factor):
    """Detect chairs in the floorplan using contour detection"""
    img = cv2.imread(image_path, 0)
    img = image.cv2_rescale_image(img, scale_factor)
    ...
    for contour in contours:
        ...
        # Chair: small, roughly square - STRICT thresholds to avoid noise
        if 2000 < area < 8000 and 0.7 < aspect_ratio < 1.5:
            chairs_list.append((x, y, w, h))

    return chairs_list


def sofas(image_path, scale_factor):
    """Detect sofas in the floorplan using contour detection"""
    img = cv2.imread(image_path, 0)
    img = image.cv2_rescale_image(img, scale_factor)
...
def kitchen_items(image_path, scale_factor):
    """Detect kitchen items (stove, sink, etc) in the floorplan using contour detection"""
    img = cv2.imread(image_path, 0)
    img = image.cv2_rescale_image(img, scale_factor)
    ...
    for contour in contours:
        ...
        # Kitchen: medium, square-ish (counters, stoves, sinks) - STRICT to avoid noise
        if 3000 < area < 15000 and 0.6 < aspect_ratio < 1.8:
            kitchen_list.append((x, y, w, h))
